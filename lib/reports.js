 ...
 * analysis.
 *
 * Strategy: prefer canonical V8 ops/sec from the deep-analysis
 * snapshot if it's present, otherwise fall back to an ops-weighted
 * average from the browser run data.
 */
function computeSummary({ benchmark, stats, analysis }) {
  ...
  const entries = tests.map((test, i) => {
    ...
    const v8Profile = aRes?.v8?.profiles?.find(p => p?.opsPerSec > 0)
    if (v8Profile?.opsPerSec) {
      opsPerSec = v8Profile.opsPerSec
      source = 'v8'
    }
...
export async function createReport({
  ...
  // Prefer the live client-supplied analysis (it carries the
  // multi-runtime + perf-counter data the persisted analysis doesn't),
  // and fall back to the most recent stored analysis otherwise.

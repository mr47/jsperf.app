...
const RESOURCE_PROFILES = [
  { label: '0.5x', resourceLevel: 0.5, memoryLimit: 8 * 1024 * 1024, vcpus: 1 },
...
export async function runAnalysis(tests, {
  ...
  for (let i = 0; i < tests.length; i++) {
    ...
    const profiles = []
    for (let profileIndex = 0; profileIndex < RESOURCE_PROFILES.length; profileIndex++) {
      const profile = RESOURCE_PROFILES[profileIndex]
      if (signal?.aborted) throw new DOMException('Aborted', 'AbortError')
  ...
  for (let i = 0; i < tests.length; i++) {
    ...
    const profiles = new Array(RESOURCE_PROFILES.length)
    // Vercel Sandbox only varies by vCPU here; avoid paying for duplicate
    // sandboxes when several display profiles share the same runtime config.
    const v8ResultsByVcpus = new Map()
    for (let profileIndex = 0; profileIndex < RESOURCE_PROFILES.length; profileIndex++) {
      const profile = RESOURCE_PROFILES[profileIndex]
      if (signal?.aborted) throw new DOMException('Aborted', 'AbortError')

      if (!v8ResultsByVcpus.has(profile.vcpus)) {
        v8ResultsByVcpus.set(profile.vcpus, await runInV8Sandbox(tests[i].code, {
          setup,
          teardown,
          timeMs: Math.min(timeMs, 1500),
          snapshotId,
      ...
      const v8Result = v8ResultsByVcpus.get(profile.vcpus)

 ...
 *
 * Takes results from QuickJS memory profiles plus canonical V8 JIT
 * profiling and produces:
 ...
 *   - Predicted ops/sec at higher memory-profile levels when available
 *   - Code characteristic badges (CPU-bound, memory-bound, JIT-friendly, etc.)
 */

import { bestFit, linearRegression } from './regression'

const PREDICTION_LEVELS = [1, 2, 4, 8]
const MIN_SCALING_CONFIDENCE = 0.5
const STABLE_PLATEAU_RELATIVE_SPREAD = 0.15
...
export function buildPrediction({ quickjsProfiles, v8Profiles }) {
  ...
  // V8 is normally a single canonical one-vCPU run. Only use it for scaling
  // if a caller explicitly supplied multiple distinct resource levels; otherwise
  // use QuickJS's memory-limit sweep for memory-response modeling.
  const { points: scalingData, source: scalingSource } = selectScalingData({ quickjsProfiles, v8Profiles, v8Available })
  ...
  return {
    ...
    scalingConfidence: scaling.confidence,
    scalingSource,
 ...
 * @returns {{ type: string, confidence: number, predict: Function | null }}
 */
function analyzeScaling(points) {
  if (points.length < 2) {
    return { type: 'insufficient-data', confidence: 0, predict: null }
  }

  const stablePlateau = detectStablePlateau(points)
  if (stablePlateau) return stablePlateau

  const fit = bestFit(points)

  if (fit.rSquared < MIN_SCALING_CONFIDENCE) {
    return { type: 'noisy', confidence: fit.rSquared, predict: null }
  }

  return { type: classifyScalingType(fit, points), confidence: fit.rSquared, predict: fit.predict }
}
...
export function buildRuntimeComparison(runtimes) {
  ...
  const entries = Object.entries(runtimes).map(([name, data]) => {
    ...
    const scaling = analyzeScaling(scalingPoints)
    ...
    return {
      ...
      profiles: profiles.map(p => ({
        ...
        latencyP50: p.latency?.p50 ?? null,
        latencyP95: p.latency?.p95 ?? null,
      ...
      scalingType: scaling.type,
      scalingConfidence: Number.isFinite(scaling.confidence) ? scaling.confidence : 0,
...
function selectScalingData({ quickjsProfiles, v8Profiles, v8Available }) {
  const v8Points = buildScalingPoints(v8Profiles)
  if (v8Points.length >= 2) {
    return { points: v8Points, source: 'v8-resource' }
  }

  const quickjsPoints = buildScalingPoints(quickjsProfiles)
  if (quickjsPoints.length >= 2) {
    return { points: quickjsPoints, source: 'quickjs-memory' }
  }

  return {
    points: v8Available ? v8Points : quickjsPoints,
    source: v8Available ? 'v8-canonical' : 'quickjs-memory',
  }
}
...
function detectStablePlateau(points) {
  const values = points
    .map(([, y]) => y)
    .filter(isPositiveFiniteNumber)
  ...
  if (!isPositiveFiniteNumber(center)) return null

  const min = Math.min(...values)
  const max = Math.max(...values)
  const relativeSpread = (max - min) / center

  if (relativeSpread > STABLE_PLATEAU_RELATIVE_SPREAD) return null

  const predicted = mean(values)
  return {
    type: 'plateau',
    confidence: clamp01(1 - relativeSpread),
    predict: () => predicted,
  }
}

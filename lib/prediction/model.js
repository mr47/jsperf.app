...
export function buildPrediction({ quickjsProfiles, v8Profiles }) {
  ...
  const jitBenefit = qjsOps > 0 ? v8Ops / qjsOps : 0

  const scalingData = v8Profiles
    .filter(p => p.resourceLevel > 0 && p.opsPerSec > 0)
    .map(p => [p.resourceLevel, p.opsPerSec])

  const scaling = analyzeScaling(scalingData)
  const memSensitivity = analyzeMemorySensitivity(v8Profiles)
  const characteristics = classifyCharacteristics({ jitBenefit, scaling, memSensitivity })
 ...
 *
 * @param {{ jitBenefit: number, scaling: object, memSensitivity: number }} data
 * @returns {{ cpuBound: boolean, memoryBound: boolean, allocationHeavy: boolean, jitFriendly: boolean }}
 */
function classifyCharacteristics({ jitBenefit, scaling, memSensitivity }) {
  return {
